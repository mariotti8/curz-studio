...
import { formatEuro, PURCHASES_ENABLED, TRIADE_PRICE } from "../lib/product";
...
export default function CartPage() {
  ...
  if (!PURCHASES_ENABLED) return <main className="inner-page commerce-page"><SiteHeader dark={false} /><section className="cart-wrap purchase-paused"><p className="section-label">Curz Studio / Triade</p><h1>{t("purchasesSoon")}</h1><p>{t("purchasePaused")}</p><Link href="/#prodotto" className="button button-light">{t("discoverTriade")}</Link></section></main>;
  return <main className="inner-page commerce-page"><SiteHeader dark={false} /><section className="cart-wrap"><p className="section-label">{t("yourOrder")}</p><h1>{t("cart")}</h1>
    {qty===0 ? <div className="empty-cart"><p>{t("emptyCart")}</p><Link href="/#prodotto" className="button button-light">{t("discoverTriade")}</Link></div> :

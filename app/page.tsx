    ...
    <main>
      ...
      <section className="hero" id="prodotto">
        ...
        <motion.div className="hero-copy" initial={reduceMotion ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : 1.15, duration: 1 }} style={{ y: heroCopyY, opacity: heroCopyOpacity }}>
          ...
          <div className="hero-actions"><a href="#design" className="button button-light">{t("discoverLamp")}</a><span className="purchase-status purchase-status--light">{t("purchasesSoon")}</span></div>
        </motion.div>
      ...
      <section className="light-experience">
        ...
        <motion.button initial={reduceMotion ? false : { opacity: 0, scale: .965 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ amount: .25 }} transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }} className={`light-compare ${before ? "is-off" : ""}`} onClick={() => setBefore(!before)} aria-pressed={before}>
          <img src={before ? "/images/triade-living-premium-off.png" : "/images/triade-living-premium.png"} alt={before ? "Lo stesso soggiorno contemporaneo con Triade spenta" : "Soggiorno contemporaneo con Triade accesa"} />
      ...
      <section className="closing" ref={closingRef}>
        ...
        <motion.div initial={reduceMotion ? false : { opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ amount: .45 }} transition={{ duration: 1 }}><p className="section-label">Triade · Curz Studio</p><h2><Multiline value={t("closingTitle")} /></h2><Link href="/design" className="button button-light">{t("discoverDesign")}</Link></motion.div>
      </section>
      <Footer /><CookieBanner />
    </main>
  );
